...
- 下拉菜单支持开关显示 **CPU / 内存 / 网络 Top N 进程（含进程名与 PID）**
- 偏好设置支持调整 **Top N 数量（1~10）**
- 基于 `/proc/stat`、`/proc/meminfo`、`/proc/net/dev` 采样，低开销且精度适合常驻显示
  - 进程排行开启后，会额外扫描 `/proc/<pid>/stat` 并调用 `ss -tinpH` 统计 TCP 连接流量

## 项目结构

- `metadata.json`：扩展元数据
- `extension.js`：顶栏 UI、采样与菜单逻辑
- `prefs.js`：GNOME 扩展首选项界面（刷新间隔、告警阈值、进程排行设置）
- `stylesheet.css`：样式
...
- 打开“显示进程排行”后，下拉菜单可在 1~2 个刷新周期内出现 CPU / 内存 / 网络 Top N 进程

## 进程排行功能说明

- 菜单内新增了 **“显示进程排行”** 开关，默认关闭
- 打开后会显示三组排行：**CPU Top / 内存 Top / 网络 Top**
- 每项展示格式为：`进程名 (PID)`
- `N` 可在扩展首选项中设置，范围为 **1~10**
- 网络排行当前基于 `ss -tinpH`，主要覆盖 **TCP 连接**；对无 TCP 连接或权限不可见的进程可能无法统计

## 开销评估

在当前机器上做了一个粗略基准（约 **662** 个进程）：

- 仅读取全部 `/proc/<pid>/stat`：约 **0.01s / 次**
- 执行一次 `ss -tinpH`：约 **0.03 ~ 0.04s / 次**

据此估算：

- **关闭进程排行时**：维持当前低开销实现，几乎不增加额外负担
- **开启进程排行且 1 秒刷新时**：相比当前版本，额外增加约 **40~50ms/秒** 的采样工作量，折合约 **4%~5% 单核 CPU 时间** 的量级
- **开启进程排行且 3 秒刷新时**：额外负担约降到 **1%~2% 单核 CPU 时间**
- **开启进程排行且 5~10 秒刷新时**：通常降到 **<1% 单核 CPU 时间**
- **常驻内存增量**：主要来自保存两次进程快照和菜单项，预计约 **数百 KiB 到 1~2 MiB**；另外 `ss` 进程会短暂产生瞬时内存占用，但不会长期驻留

如果后续需要进一步压低开销，建议优先做两点优化：

1. 将网络进程排行刷新频率降为低于主刷新频率（如每 3~5 秒采样一次）
2. 将网络统计从外部 `ss` 命令迁移为更底层的 `/proc` / `netlink` 方案，减少子进程启动成本
